...
func get_display_data() -> Dictionary:
	...
	var catalog := campaign.get_shop_catalog(selected_category)
	var bolts := campaign.get_bolts()
	var owned := campaign.get_owned_items(selected_category)
...
func try_buy(item_id: String) -> Dictionary:
	...
	var result := campaign.buy_item(selected_category, item_id)
	if result["success"]:
