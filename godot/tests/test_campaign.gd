...
#
# Tests: new game, shop, loadout, opponent select, match, rewards, progression.
extends RefCounted

const CampaignController = preload("res://game/campaign/campaign_controller.gd")
const ShopScreen = preload("res://game/ui/shop_screen.gd")
const OpponentSelect = preload("res://game/ui/opponent_select.gd")

func _make() -> CampaignController:
	var c := CampaignController.new()
	c.new_game()
	return c
...
func test_new_game_starts_at_shop() -> bool:
	var c := _make()
	return c.current_phase == CampaignController.Phase.SHOP
...
func test_match_produces_result() -> bool:
	var c := _make()
	c.set_loadout("scout", ["minigun"], "plating", [])
	c.start_match(0)
	return c.last_match_result.has("winner_team") and c.last_match_result.has("outcome")

func test_match_phase_transitions() -> bool:
	var c := _make()
	c.set_loadout("scout", ["minigun"], "plating", [])
	c.start_match(0)
	# After match completes, should be in RESULT phase
	return c.current_phase == CampaignController.Phase.RESULT
